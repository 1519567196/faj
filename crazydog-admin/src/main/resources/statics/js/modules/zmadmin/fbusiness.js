$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/fbusiness/list',
        datatype: "json",
        colModel: [			
			{ label: 'itemid', name: 'itemid', index: 'itemid', width: 50, key: true,hidden:true },
			{ label: '联系人姓名', name: 'memberName', index: 'member_name', width: 80 }, 			
			{ label: '电话', name: 'memberMobile', index: 'member_mobile', width: 80 }, 			
			{ label: '公司名称', name: 'company', index: 'company', width: 80 }, 			
			{ label: '公司地址', name: 'address', index: 'address', width: 80 }, 			
			{ label: '省', name: 'privinceid', index: 'privinceid', width: 80 },
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.fBusiness.itemid == null ? "sys/fbusiness/save" : "sys/fbusiness/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.fBusiness),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "sys/fbusiness/delete",
                    contentType: "application/json",
		...
		getInfo: function(itemid){
			$.get(baseURL + "sys/fbusiness/info/"+itemid, function(r){
                vm.fBusiness = r.fBusiness;
            });
		},
